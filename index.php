...
if(isset($_SESSION['Auth']) && $user['ac_status'] == 1 && !$pagecount){
    ...
    showPage('header', ['page_title' => 'Account Blocked']);
    showPage('blocked');
    ...
    if(!$profile){
        ...
        $profile_note = getNoteById($profile['id']);  
    ...
    showPage('header', ['page_title' => 'NoteFlix - Sign Up']);
    showPage('signup');
} elseif (isset($_GET['login'])) {
    showPage('header', ['page_title' => 'NoteFlix - Login']);
    showPage('login');
} elseif (isset($_GET['forgotpassword'])) {
    showPage('header', ['page_title' => 'NoteFlix - Reset Password']);
